...
var Promise, async, config, fs, logPrefix, migrator;
  ...
  migrate: function() {
    ...
    createMigrationTable = function() {
      return new Promise(function(resolve, reject) {
        return db.query('select count(*) as cnt from information_schema.tables where lower(table_schema) = lower(?) and lower(table_name) = lower(?)', [config.database, migrator.tableName], function(err, rows) {
          if (err) {
            return reject(err);
          }
          if (rows[0].cnt) {
